...
var passwordProtected = hexo.config.admin && hexo.config.admin.lock;
...
if (passwordProtected) {
	...
	if (!hexo.config.admin.secret) {
		console.error(
			"[Hexo Admin]: config admin.secret is requred for authentication"
		);
		passwordProtected = false;
	}
}
...
hexo.extend.filter.register("server_middleware", function (app) {
	...
	app.use(
		...
		serveStatic(path.join(__dirname, "www"))
	);
